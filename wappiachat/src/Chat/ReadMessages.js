...
class ReadMessages extends Component {
    ...
    render() { 
        let messages = this.state.sendedMessages.map((message,index) => (
            <ul style={message.messages.from === "1" ? {textAlign:"left"} : {textAlign:"center"} } key={"messagesContainer" + index}>
        ...
        return ( 
            <React.Fragment>
                <button className="btn btn-info" onClick={()=>this.updateMessages()}>Update</button>
                <div id="readMessagesBar">
